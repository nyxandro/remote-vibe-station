/**
 * @fileoverview Sync default Telegram media plugin assets into the persisted OpenCode config volume.
 *
 * Exports:
 * - ensureTelegramMediaPluginAssets - Copies plugin source and merges required package dependencies.
 */

const fs = require("node:fs");
const path = require("node:path");

const PLUGIN_FILE_NAME = "telegram-media-tools.ts";
...
const ensureTelegramMediaPluginAssets = ({ configDir, pluginSourcePath }) => {
  /* OpenCode loads local plugins from ~/.config/opencode/plugins at startup. */
  if (typeof configDir !== "string" || configDir.trim().length === 0) {
    throw new TypeError("configDir must be a non-empty string for telegram media plugin sync");
  }
  ...
  if (!fs.existsSync(pluginSourcePath)) {
    throw new Error(`Telegram media plugin source file not found: ${pluginSourcePath}`);
  }
...
if (require.main === module) {
  ...
  ensureTelegramMediaPluginAssets({ configDir, pluginSourcePath });
}

module.exports = {
  ensureTelegramMediaPluginAssets
};
