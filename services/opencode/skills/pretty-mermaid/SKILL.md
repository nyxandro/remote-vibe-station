...
When the user wants the rendered diagram sent back in Telegram:

1. Save temporary Mermaid artifacts under a dedicated temporary folder such as `.opencode/tmp/mermaid/` inside the current project.
2. Render the diagram to `.svg` first. If Telegram delivery needs an image preview, also create a `.png` copy from that SVG.
3. Send the final file with the existing `telegram_send_media` tool instead of inventing a custom upload flow.
4. After successful Telegram delivery, delete the temporary `.mmd`, `.svg`, `.png`, and any other intermediate files so the workspace does not accumulate artifacts.
5. If delivery fails, keep the generated file and report its exact path to the user for manual inspection or retry.
