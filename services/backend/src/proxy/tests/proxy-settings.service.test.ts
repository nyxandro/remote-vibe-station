...
describe("ProxySettingsService", () => {
  test("generates proxy env and override files when runtime dir is configured", async () => {
    ...
    try {
      ...
      expect(overrideContent).toContain('command: ["run", "-c", "/etc/xray/xray.json"]');
      expect(overrideContent).toContain('./infra/vless:/etc/xray:ro');
      expect(overrideContent).not.toContain('entrypoint: ["sh", "-lc"]');
